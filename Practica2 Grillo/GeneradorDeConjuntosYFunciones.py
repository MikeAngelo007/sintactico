...
with open( 'AnalizadorSintactico.py', 'w' ) as f:
	...
	print t +"\"and\" : \"token_y\","
	print t +"\"or\" : \"token_o\","
	print t +"\"not\" : \"token_neg\","
	print t +"\"xor\" : \"token_xor\","
...
#creador de las funciones
	for noTerminal in noTerminales:
		...
		for i in predicciones:
			...
			if noTerminal == trans[ 0 ]:
				if cont != 0:
					t += "el"
				print t + "if token in", predicciones[i], ":"
				if cont != 0:
					t = t[ : 1 ]
				t += "\t"
		...
		print t + "else:"
		t += "\t"
		print t + "errorSintaxis(", primeros[ noTerminal ], " )\n"
	print ""
	print "def errorSintaxis( lista ):"
	t = "\t"
	print t + "global stopProcess"
	print t + "print '<' + str( fila ) + ':' + str( columna ) + '> Error sintactico: se encontro: ' + token + '; se esperaba: '' + '', ''.join( lista ) + ''.'"
	...
	print t + "elif caracter in range( 97 , 123 ) and subs != \"mod\":"
	print t + "\treturn \"logico\""
	...
	print t + "else: return \"aritmetico\""
	...
	print t + "simbolType = simb( simbol )" 
	...
	print t + "tok = TOKEN( subs[ 1 : len( subs ) - 1 ] , \"string\" , str( numeroLinea ) , str ( ind + 1 ) )"
	print t + "TOKENS.append( tok )"
	t = t[ 1 : ]
	print t + "else:"
	t += "\t"
	print t + "#print '<id,' + subs.lower() + ',' + str( numeroLinea ) +',' + str( ind + 1 ) + '>'"
	print t + "tok = TOKEN( subs.lower() , \"id\" , str( numeroLinea ) , str ( ind + 1 ) )"
	print t + "TOKENS.append( tok )"
	...
	print t + "tok = TOKEN( subs , \"long\" , str( numeroLinea ) , str( ind + 1 ) )"
	print t + "TOKENS.append( tok )"
	t = "\t"
	print t + "else:"
	t += "\t"
	print t + "#print '<token_integer,' + subs + ',' + str( numeroLinea ) +',' + str( ind + 1 ) + '>'"
	print t + "tok = TOKEN( subs , \"integer\" , str( numeroLinea ) , str( ind + 1 ) )"
	print t + "TOKENS.append( tok )"
	...
	print t + "tok = TOKEN( subs , \"double\" , str( numeroLinea ) , str( ind + 1 ) )"
	print t + "TOKENS.append( tok )"
	t = "\t"
	print t + "else:"
	t += "\t"
	print t + "#print '<token_single,' + subs + ',' + str( numeroLinea ) +',' + str( ind + 1 ) + '>'"
	print t + "tok = TOKEN( subs , \"single\" , str( numeroLinea ) , str( ind + 1 ) )"
	print t + "TOKENS.append( tok )"
	t = "\t"
	...
	print ""
	print "print \"size de la lista de TOKENS:\", len( TOKENS )"
	print "for tok in TOKENS:"
	print t + "print tok.token , tok.tipo , tok.fila , tok.columna"
